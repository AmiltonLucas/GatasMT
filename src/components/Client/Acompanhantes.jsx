...
export default function Acompanhantes({ items = null, onChat, onWhatsApp }) {
  ...
  const filteredList = useMemo(() => {
    ...
    const getPrice = (p) => parseInt((p.price || "").replace(/\D/g, "")) || 0;

    let result = list.filter((p) => {
      const price = getPrice(p);

      if (filters.priceMin && price < (parseInt(filters.priceMin) || 0))
        return false;
      if (filters.priceMax && price > (parseInt(filters.priceMax) || Infinity))
        return false;
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="md:flex md:items-start md:gap-6">
          ...
          <aside className="hidden md:block md:w-80">
            <Filtro onFilterChange={handleFilterChange} onClear={handleClear} />
          </aside>

          {/* Main content */}
          <main className="flex-1">
            <h2
              className={`text-2xl font-bold mb-4 text-center  ${COLORS.title}`}
            >
              Acompanhantes
            </h2>
